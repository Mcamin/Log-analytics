...
def main():
    ...
    db = load_logs("news")
    c = db.cursor()
    Rates = "".join(errorRate % (rate, date.strftime('%d, %b %Y'), rate) for rate, date in get_requests_errors(c))
    Authors = "".join(popularAuthors % (author, views) for author, views in get_popular_authors(c))
    Articles = "".join(popularArticles % (title, visits) for title, visits in get_popular_articles(c))
...
def getTextFile():
    ...
    t += "\n".join("%s - %s Views " % (author, views) for author, views in get_popular_authors(c))
    t += "\n\n2. Who are the most popular article authors of all time?\n"
    t += "\n".join("%s - %s Views " % (title, visits) for title, visits in get_popular_articles(c))
    t += "\n\n3. On which days did more than 1% of requests lead to errors?\n"
    t += "\n".join("%s%% - %s " % (rate, date.strftime('%d, %b %Y')) for rate, date in get_requests_errors(c))
    close_db(db)
    print(t)
    return Response(
